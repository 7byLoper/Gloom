...
public class HistoryMenuPlaceholders {
    ...
    public static ItemStack buildSnapshotItem(
            MenuItem menuItem, ProbSnapshot snapshot, int snapshotNumber, HistoryMenu menu) {
        double average = getAverage(snapshot);
        ItemStack itemStack =
                ItemPlaceholderUtils.buildMenuItem(menuItem, snapshotPlaceholders(snapshot, snapshotNumber, menu));
        itemStack.setType(menu.getHistoryMenuSettings().getChanceMaterial(average));
        return itemStack;
    }

    public static Map<String, String> menuPlaceholders(String playerName, int page, int maxPages, int snapshotsCount) {
    ...
    public static Map<String, String> snapshotPlaceholders(
            ProbSnapshot snapshot, int snapshotNumber, HistoryMenu menu) {
    ...
    private static double getAverage(List<Prob> entries) {
        return entries.stream().mapToDouble(Prob::getChance).average().orElse(0.0D);
    }
    ...
    private static List<Prob> getEntries(ProbSnapshot snapshot) {
        ...
        return snapshot.getProbabilities().stream().filter(Objects::nonNull).toList();
    }
