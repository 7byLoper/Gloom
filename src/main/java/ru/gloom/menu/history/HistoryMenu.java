...
public class HistoryMenu extends ActionMenu {
    ...
    public @Nullable String getTitle() {
        return ItemPlaceholderUtils.apply(historyMenuSettings.getTitle(), getMenuPlaceholders());
    }
    ...
    protected void onLoad() {
        ...
        maxPages = Math.max(1, (int) Math.ceil(
                snapshotsCount / (double) historyMenuSettings.getSnapshotSlots().size()));
        page = Math.max(0, Math.min(page, maxPages - 1));

        int fromIndex = page * historyMenuSettings.getSnapshotSlots().size();
        int toIndex =
                Math.min(fromIndex + historyMenuSettings.getSnapshotSlots().size(), snapshots.size());
    ...
    public Map<String, String> getMenuPlaceholders() {
        return HistoryMenuPlaceholders.menuPlaceholders(playerName, page, maxPages, snapshotsCount);
    }
    ...
    private PlayerAIProbabilityData loadPlayerData() {
        ...
        try {
            ...
            Bukkit.getLogger()
                    .log(Level.SEVERE, "Не удалось загрузить историю вероятностей игрока " + playerName, exception);
        }
    ...
    private List<SnapshotView> createSnapshotViews(PlayerAIProbabilityData data) {
        ...
        snapshots.sort(Comparator.comparingLong((SnapshotView view) -> getSnapshotTimestamp(view.snapshot()))
