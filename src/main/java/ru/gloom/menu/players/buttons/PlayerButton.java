...
public class PlayerButton extends Button {
    ...
    public PlayerButton(MenuItem menuItem, PlayerAIProbabilityData playerData, int slot, PlayersMenu menu) {
        super(
                PlayersMenuPlaceholders.buildPlayerItem(
                        menuItem,
                        playerData,
        ...
        this.placeholders = PlayersMenuPlaceholders.playerPlaceholders(
                playerData, menu.getPlayersMenuSettings(), menu.getPage(), menu.getMaxPages(), menu.getPlayersCount());
    }
    ...
    private List<String> getActions(ClickType clickType) {
        return menuItem.getSection()
                .getStringList("click_actions." + clickType.name().toLowerCase());
    }
    ...
    private void executeAction(Player player, String rawAction) {
        ...
        if (action.startsWith("[message]")) {
            player.sendMessage(
                    StringColorize.parse(action.substring("[message]".length()).trim()));
            return;
        }
