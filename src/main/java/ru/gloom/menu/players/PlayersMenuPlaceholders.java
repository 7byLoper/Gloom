...
public class PlayersMenuPlaceholders {
    ...
    public static ItemStack buildPlayerItem(
            ...
            int maxPages,
            int playersCount) {
        Map<String, String> placeholders = playerPlaceholders(playerData, settings, page, maxPages, playersCount);
    ...
    public static Map<String, String> playerPlaceholders(
        ...
        double lastChance = shownProbabilities.isEmpty() ? 0.0D : shownProbabilities.get(shownProbabilities.size() - 1);
        double avgChance = average(shownProbabilities);
        double maxChance = shownProbabilities.stream()
                .mapToDouble(Double::doubleValue)
                .max()
                .orElse(0.0D);
        double minChance = shownProbabilities.stream()
                .mapToDouble(Double::doubleValue)
                .min()
                .orElse(0.0D);
        ...
        placeholders.put(
                "max_probability", GloomAI.INSTANCE.getMainConfigManager().getChanceString(maxChance));
        placeholders.put(
                "min_probability", GloomAI.INSTANCE.getMainConfigManager().getChanceString(minChance));
    ...
    private static double average(List<Double> probabilities) {
        return probabilities.stream().mapToDouble(Double::doubleValue).average().orElse(0.0D);
    }
