...
public final class PunishmentManager {
    ...
    private void executeAction(
        ...
        switch (trimmedAction.toLowerCase(Locale.ROOT)) {
            ...
            default ->
                executeConsoleCommand(trimmedAction, check, verbose, aiProbability, checkViolations, totalViolations);
        }
    }
    ...
    private void executeAiAlert(Check check, double aiProbability) {
        ...
        String message = GloomAI.INSTANCE
                .getMainConfigManager()
                .getAiAlertMessage()
                .replace("{player}", check.getPlayer().getName())
                .replace("{vl}", String.valueOf((int) check.getViolations()))
    ...
    private void executeConsoleCommand(
            ...
            int totalViolations) {
        String command =
                applyPlaceholders(commandTemplate, check, verbose, aiProbability, checkViolations, totalViolations);
    ...
    private String applyPlaceholders(
        ...
        return template.replace("{player}", check.getPlayer().getName())
    ...
    private static final class ActivePunishGroup {
        ...
        private ActivePunishGroup(
                String name,
                List<String> checkPatterns,
                List<ParsedRuntimeCommand> commands,
        ...
        public int getViolationsForCheck(String checkName) {
            return (int) violations.values().stream().filter(checkName::equals).count();
        }
