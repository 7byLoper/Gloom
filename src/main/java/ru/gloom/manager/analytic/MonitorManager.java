...
import ru.gloom.api.model.monitor.AiSnapshot;
import ru.gloom.api.model.monitor.MonitorSession;
import ru.gloom.api.model.monitor.ToggleResult;
...
public final class MonitorManager {
    ...
    public ToggleResult toggle(Player viewer, Player target) {
        ...
        ToggleResult result = currentSession == null ? ToggleResult.ENABLED : ToggleResult.SWITCHED;
    ...
    public void publish(Player target, double probability, double buffer) {
        ...
        snapshots.compute(targetId, (ignored, previous) -> {
            ...
            return new AiSnapshot(normalizedProbability, normalizedBuffer, trend);
        });
    }
    ...
    public void startUpdaterIfNeeded() {
        ...
        currentTick = 0L;
        updateTask = Bukkit.getScheduler().runTaskTimer(plugin, this::tick, 1L, UPDATE_PERIOD_TICKS);
    }

    private void tick() {
        currentTick += UPDATE_PERIOD_TICKS;

        Iterator<Map.Entry<UUID, MonitorSession>> iterator = sessions.entrySet().iterator();
        ...
        while (iterator.hasNext()) {
            ...
            if (target == null || !target.isOnline()) {
                ...
                viewer.sendMessage(replacePlayer(config.getMonitorTargetLeftMessage(), session.getTargetName()));
                continue;
            }

            String actionBar = createActionBar(target, snapshots.get(session.getTargetId()));
            if (!session.shouldSend(actionBar, currentTick, ACTION_BAR_KEEP_ALIVE_TICKS)) {
                continue;
            }
    ...
    private String createActionBar(Player target, AiSnapshot snapshot) {
        if (snapshot == null) {
            return replacePlayer(config.getMonitorWaitingFormat(), target.getName());
        }
