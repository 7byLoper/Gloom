...
public class ViolationDatabase implements ViolationStorage {
    ...
    public CompletableFuture<Void> createTable() {
        return CompletableFuture.runAsync(
                () -> {
                    String query = "CREATE TABLE IF NOT EXISTS violations (" + "id INT AUTO_INCREMENT PRIMARY KEY, "
    ...
    @Override
    public CompletableFuture<Void> logAlert(
            UUID uniqueId, String acVersion, String verbose, String checkName, int vls) {
        return CompletableFuture.runAsync(
                () -> {
                    String query =
                            "INSERT INTO violations (player_uuid, ac_version, verbose, check_name, vls, server, timestamp) "
                                    + "VALUES (?, ?, ?, ?, ?, ?, ?);";
                    ...
                    try (Connection conn = databaseManager.getConnection();
                        ...
                        GloomAI.INSTANCE
                                .getLogger()
                                .log(Level.SEVERE, "Failed to log alert for player: " + uniqueId, e);
                    }
                },
                executorService);
    }
        ...
        return CompletableFuture.supplyAsync(
                () -> {
                    ...
                    try (Connection conn = databaseManager.getConnection();
                        ...
                        GloomAI.INSTANCE
                                .getLogger()
                                .log(Level.SEVERE, "Failed to get log count for UUID: " + uniqueId, e);
                    }
                    return 0;
                },
                executorService);
    ...
    public CompletableFuture<List<ViolationRecord>> getViolations(UUID uniqueId, int page, int limit) {
        return CompletableFuture.supplyAsync(
                () -> {
                    ...
                    String query =
                            "SELECT * FROM violations WHERE player_uuid = ? ORDER BY timestamp DESC LIMIT ? OFFSET ?;";
                    ...
                    try (Connection conn = databaseManager.getConnection();
                        ...
                        GloomAI.INSTANCE
                                .getLogger()
                                .log(Level.SEVERE, "Failed to get violations for UUID: " + uniqueId, e);
                    }
                    return violations;
                },
                executorService);
