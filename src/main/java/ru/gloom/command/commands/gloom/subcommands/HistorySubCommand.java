...
public class HistorySubCommand implements BuildableCommand {
    ...
    private void openHistoryMenu(@NotNull CommandSender sender, @NotNull String targetName) {
        ...
        dataFuture.whenComplete((playerData, throwable) -> Bukkit.getScheduler().runTask(GloomAI.INSTANCE, () -> {
            if (!player.isOnline()) {
                return;
            }
            ...
            if (playerData == null) {
                player.sendMessage(config.getHistoryNoDataMessage().replace("{player}", targetName));
                return;
            }
