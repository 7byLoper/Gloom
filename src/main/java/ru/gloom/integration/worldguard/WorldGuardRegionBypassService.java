...
public final class WorldGuardRegionBypassService {
    ...
    public boolean isBypassed(Player player) {
        ...
        Set<String> disabledRegions = WorldGuardRegionMatcher.getRegions(player, config.getDisabledRegionsByWorld());
        if (disabledRegions.isEmpty()) {
            return false;
        }

        Set<String> enabledRegions = WorldGuardRegionMatcher.getRegions(player, config.getEnabledRegionsByWorld());
        ...
        return disabledRegions.contains(WorldGuardRegionMatcher.GLOBAL_WORLD) || hasAny(activeRegions, disabledRegions);
    }
