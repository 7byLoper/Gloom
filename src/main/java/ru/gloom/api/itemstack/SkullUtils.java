...
public class SkullUtils {
    ...
    public static ItemStack getSkullByBase64EncodedTextureUrl(@NotNull final String base64Url) {
        ...
        try {
            ...
            Bukkit.getLogger().log(Level.SEVERE, "Failed to get head item from base64 texture url", exception);
        }
        head.setItemMeta(headMeta);
        return head;
    }
    ...
    public static String getTextureFromSkull(ItemStack item) {
        ...
        try {
            ...
            Bukkit.getLogger().log(Level.SEVERE, "Failed to get base64 texture url from head item", exception);
            return null;
        }
    ...
    public static ItemStack getSkullByName(@NotNull final String playerName) {
        ...
        if (VersionHelper.HAS_PLAYER_PROFILES
                && offlinePlayer.getPlayerProfile().getTextures().isEmpty()) {
            // updates the Player Profile and populates textures for offline players - for some reason this doesn't
            // populate when getting the Profile first time
