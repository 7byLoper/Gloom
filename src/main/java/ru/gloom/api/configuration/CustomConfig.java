...
public class CustomConfig {
    ...
    public void reloadConfig() {
        try {
            ...
            GloomAI.INSTANCE
                    .getLogger()
                    .log(Level.WARNING, "Ошибка чтения конфига {0}: {1}", new Object[] {name, e.getMessage()});
        } catch (org.bukkit.configuration.InvalidConfigurationException e) {
            GloomAI.INSTANCE
                    .getLogger()
                    .log(Level.WARNING, "Некорректный формат конфига {0}: {1}", new Object[] {name, e.getMessage()});
        }
    }
