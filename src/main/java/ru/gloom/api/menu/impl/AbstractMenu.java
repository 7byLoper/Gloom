...
public abstract class AbstractMenu implements InventoryHolder, IMenu {
    ...
    public boolean isSlotOccupied(int slot) {
        return menuItems.containsKey(slot)
                || menuButtons.stream().anyMatch(button -> button.getSlots().contains(slot));
    }

    public void addDecorItems(@NotNull Material material, @NotNull List<Integer> slots) {
        ItemStack decor =
                new ItemBuilder(material).flags(ItemFlag.HIDE_ATTRIBUTES).build();
    ...
    public void addDecorFromSection(@NotNull ConfigurationSection section) {
        section.getKeys(false).forEach(key -> {
            try {
                ...
                Bukkit.getLogger().log(Level.SEVERE, "Ошибка при загрузке декорации для меню", e);
            }
        });
    }
